...
int main() {
	...
	for (int i = 0; i < model.m_group_table.Count(); i++) {
		auto id = model.m_group_table[i].GroupIndex();
		if (group_vertices.count(id) == 0) {
			continue;
		}
		char filename[100];
		sprintf(filename, "full\\%d.obj\0", id);
		write_obj(group_vertices[id], group_faces[id], filename);

		sprintf(filename, "simplified\\%d.obj\0", id);
		write_obj(simplified_group_vertices[id], simplified_group_faces[id], filename);

		//append_mesh(fvs, ffs, simplified_group_vertices[id], simplified_group_faces[id]);
	}
